...
<body>
    <div class="container-fluid p-4">
        ...
        <div class="no-print text-center mb-3">
            ...
            $laThanhToanSau = $hoaDon->trang_thai == 'da_thanh_toan' && ($phuongThucTT == 'chua_thanh_toan' ||
            !$phuongThucTT);
            @endphp
            @if($laThanhToanSau)
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>
                        <strong>
                            @php
                            // Chỉ hiển thị "Đã thanh toán" khi đã thanh toán nhưng không biết phương thức (thanh toán
                            sau)
                            $chiTiet = $hoaDon->chiTietHoaDon;
                            $phuongThucTT = $chiTiet ? ($chiTiet->phuong_thuc_tt ?? null) : ($hoaDon->phuong_thuc_tt ??
                            null);

                            if($hoaDon->trang_thai == 'da_thanh_toan' && ($phuongThucTT == 'chua_thanh_toan' ||
                            !$phuongThucTT)) {
        ...
        <table class="invoice-table">
            ...
            <tbody>
                ...
                if($soTreEm > 0 && $soNguoiDaXuLy < $soTreEm) { $soTreEmConLai=$soTreEm - $soNguoiDaXuLy;
                    ...
                    $soNguoiDuocGiam; // Hiển thị combo giảm giá (trẻ em) nếu có if($soNguoiDuocGiam> 0) {
                    ...
                    <tr>
                        ...
                        <td class="text-end" data-label="Đơn giá">
                            <span class="text-decoration-line-through"
                                style="font-size: 11px;">{{ number_format($giaComboGoc) }} đ</span>
                            <br>
                    ...
                    // Tính số lượng theo trạng thái: chỉ tính đã lên + đang nấu + chờ cung ứng (không tính hủy, chờ
                    bếp)
                    $soLuongDaLen = 0;
                    ...
                    if($tongSoLuongHienThi <= 0) { continue; } // Lấy đơn giá $donGiaGoc=0; if($monAnGroup &&
                        $monAnGroup->first()->monAn) {
                        $donGiaGoc = $monAnGroup->first()->monAn->gia ?? 0;
                        }

                        // Tính thành tiền: đã lên + đang nấu + chờ cung ứng
                        $thanhTienTinhLai = ($donGiaGoc * $soLuongDaLen) + ($donGiaGoc * $soLuongDangCheBien) +
                        ($donGiaGoc * $soLuongChoCungUng);
                        @endphp
                        <tr>
                        ...
                        if($tongGioiHan !== null && $tongGioiHan > 0 && $tongSoLuong <= $tongGioiHan) {
                            $coTrongCombo=true; // Món trong combo, không vượt giới hạn } // Chỉ hiển thị món gọi thêm
                            (không phải món combo) if($coTrongCombo) { continue; } // Tính số lượng theo trạng thái: chỉ
                            tính đã lên + đang nấu + chờ cung ứng (không tính hủy, chờ bếp) $soLuongDaLen=0;
                            $soLuongDangCheBien=0; $soLuongChoCungUng=0; foreach($monAnGroup as $ct) { if($ct->
                            trang_thai == 'da_len_mon') {
                            $soLuongDaLen += $ct->so_luong;
                            } elseif($ct->trang_thai == 'dang_che_bien') {
                            $soLuongDangCheBien += $ct->so_luong;
                            } elseif($ct->trang_thai == 'cho_cung_ung') {
                            ...
                            if($tongSoLuongHienThi <= 0) { continue; } // Lấy đơn giá $donGiaGoc=0; if($monAnGroup &&
                                ...
                                @endif
                                <tr
                                    style="background-color: #e0e0e0; font-weight: bold; font-size: 16px; border-top: 2px solid #000;">
        ...
        <div class="invoice-summary">
            <table>
                ...
                $tongTienSauVoucher = $tongTienComboMon - (($chiTiet && $chiTiet->tien_giam_voucher) ?
                $chiTiet->tien_giam_voucher : ($hoaDon->tien_giam ?? 0));
                if($tongTienSauVoucher < 0) $tongTienSauVoucher=0; // Tính phải thanh toán: tổng tiền sau voucher - tiền
                    cọc $phaiThanhToan=$tongTienSauVoucher - (($chiTiet && $chiTiet->tien_coc) ? $chiTiet->tien_coc :
                    ($hoaDon->datBan->tien_coc ?? 0));
                    if($phaiThanhToan < 0) $phaiThanhToan=0; @endphp @if($tongTienCombo> 0)
                        <tr>
                            <td>Tiền combo:</td>
                        ...
                        <tr>
                            <td>(-) Tiền cọc:</td>
                            <td>- {{ number_format($chiTiet ? $chiTiet->tien_coc : ($hoaDon->datBan->tien_coc ?? 0)) }}
                                đ</td>
                        </tr>
                        @endif
                        <tr class="total-row">
